...
Python CLI for downloading authorized O'Reilly Learning book assets and rebuilding
them into local EPUB files.
...
- Build: `poetry build`
- Test (all): no committed test suite yet; use targeted CLI/module checks
- Test (single file): no committed test suite yet
- Lint: `poetry run pre-commit run --all-files`
- Typecheck: no configured typechecker
- Run locally: `poetry run oreilly-library --help`

Prefer single-file or single-test runs during iteration. Full suites are for the final verification pass.

### Layout

- Source lives in: `src/oreilly_library/`
- Tests live in: no committed `tests/` directory yet
- Do not modify: downloaded sample book assets under `example/` unless the task is explicitly about fixtures or sample EPUB content

### Conventions specific to this repo

- Naming: public CLI command is `oreilly-library`; package modules use snake_case under `oreilly_library`
- Import style: `from __future__ import annotations` in package modules; standard library, third-party, then local imports
- Error handling pattern: raise specific exceptions for invalid CLI combinations or missing local inputs; print non-fatal EPUB cleanup warnings through the builder warning path
- Testing pattern and framework: no framework is committed; validate changes with focused commands such as local rebuilds from `example/`, `poetry run oreilly-library --build --output-dir . 0642572230302`, and `python check_epub_unicode.py <book.epub>`

### Forbidden

- Do not run authenticated O'Reilly downloads unless the user explicitly asks and provides/has configured valid access.
- Do not assume `--clean` or `--epubcheck` are available; they depend on external Calibre and epubcheck executables.
- Do not commit generated EPUBs, downloaded book directories, cookies, or local working output unless the user explicitly requests fixture updates.
